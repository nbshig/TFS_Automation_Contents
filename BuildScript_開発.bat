...
CLS
@ECHO **********************************************
@ECHO 対象のソリューション構成No.を入力してください
@ECHO **********************************************
TYPE %~dp0Sub_Module\Const_Sln.ini
@ECHO .
:ConstSln_Return
SET /P BUILD_NO=No：
IF %BUILD_NO%==Nothing @ECHO Noが入力されていません & GOTO ConstSln_Return
...
REM //VSWebCache削除
@ECHO VSWebCacheフォルダを削除 >> %LOG_DIR%\%LOGFILE%
@ECHO VSWebCacheフォルダを削除
%~dp0Sub_Module\DelWebCache.vbs >> %LOG_DIR%\%LOGFILE%
IF ERRORLEVEL 1 (
SET TOTAL_ERRFLG=1
@ECHO VSWebCacheフォルダ削除_NG >> %LOG_DIR%\%LOGFILE%
@ECHO VSWebCacheフォルダ削除_NG
@ECHO ---- ＳＣＲＩＰＴ ＥＲＲＯＲ ！！！！ ----
@ECHO VSWebCacheフォルダを削除後、何かキーを押して先に進んでください
@ECHO VSWebCacheフォルダパス：C:\Users\[アカウント名]\VSWebCache
PAUSE
) ELSE (
@ECHO VSWebCacheフォルダ削除_OK >> %LOG_DIR%\%LOGFILE%
@ECHO VSWebCacheフォルダ削除_OK
)
...
@ECHO -------------------------------------------------------------------------- >> %LOG_DIR%\%LOGFILE%
@ECHO --------------------------------------------------------------------------
@ECHO ビルドソースバックアップ >> %LOG_DIR%\%LOGFILE%
@ECHO ビルドソースバックアップ
CALL :PROC_COPY %SOLUTION_DIR% %SOURCE_BACKUP%\Source\
IF %ERRFLG%==1 (
SET TOTAL_ERRFLG=1
@ECHO ビルドソースバックアップ_NG >> %LOG_DIR%\%LOGFILE%
...
@ECHO ビルドソースバックアップ_OK >> %LOG_DIR%\%LOGFILE%
...
@ECHO -------------------------------------------------------------------------- >> %LOG_DIR%\%LOGFILE%
@ECHO --------------------------------------------------------------------------
@ECHO InstallImageテンポラリフォルダ作成 >> %LOG_DIR%\%LOGFILE%
...
IF %ERRFLG%==1 (
SET TOTAL_ERRFLG=1
@ECHO InstallImageテンポラリフォルダ作成_NG >> %LOG_DIR%\%LOGFILE%
...
@ECHO InstallImageテンポラリフォルダ作成_OK >> %LOG_DIR%\%LOGFILE%
...
@ECHO -------------------------------------------------------------------------- >> %LOG_DIR%\%LOGFILE%
@ECHO --------------------------------------------------------------------------
@ECHO binフォルダ作成 >> %LOG_DIR%\%LOGFILE%
@ECHO binフォルダ作成
...
@ECHO binフォルダ作成_OK >> %LOG_DIR%\%LOGFILE%
@ECHO binフォルダ作成_OK
)
